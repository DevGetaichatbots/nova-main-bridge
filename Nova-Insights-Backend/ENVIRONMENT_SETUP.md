...
> [!IMPORTANT]
> **Azure PostgreSQL requires SSL.** Ensure that `?sslmode=require` is appended to your `DATABASE_URL` or that `DB_SSLMODE=require` is set if using individual variables.
