          ...
          <div className="p-3 border-b border-gray-100">
            ...
            >
              <span className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${
                isReadOnly ? 'bg-gray-300' : isCreatingChat ? 'bg-[#1eb5ee]/60' : 'bg-[#1eb5ee]'
              }`}>
                {isCreatingChat ? (
                      ...
                      className={`group relative rounded-lg transition-all duration-300 ${
                        isBeingDeleted
                          ...
                          : activeSessionId === session.session_id
                            ? 'bg-[#1eb5ee]/10 border border-[#1eb5ee]/30'
                        ...
                        <div className="p-2">
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              className="flex-1 text-xs px-2 py-1.5 border border-[#1eb5ee] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#1eb5ee]"
                              maxLength={255}
                              disabled={isRenaming}
                            />
                            <button
                              ...
                              disabled={isRenaming}
                              className="p-1 bg-[#1eb5ee] text-white rounded hover:bg-[#00B8B8] transition-colors min-w-[24px] flex items-center justify-center"
                              title={t('common.save')}
                            >
                              {isRenaming ? (
                          ...
                          {!isBeingDeleted && deleteConfirmId === session.session_id ? (
                            ...
                            <div className="absolute right-1 top-1/2 -translate-y-1/2 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-all">
                              <button
                                ...
                                className="p-1 text-gray-400 hover:text-[#1eb5ee] hover:bg-[#1eb5ee]/10 rounded transition-colors"
      ...
      {isOpen && (
        ...
        >
          <div className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-8 rounded-full flex items-center justify-center transition-opacity ${isResizing ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
            style={{ background: 'linear-gradient(135deg, #1eb5ee 0%, #00B8B8 100%)' }}
          >
