...
function groupRowsByCategory(tables) {
  ...
  const groups = {
    ...
    default: { rows: [], label: 'Other Tasks', labelDa: 'Andre Opgaver', colorClass: 'text-sky-500', bgClass: 'bg-sky-50/50', borderClass: 'border-sky-200' }
  };
...
const GroupedTable = ({ groupedData, tables, onAddComment, activeFilter = 'all', onFilterChange }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg shadow-sky-500/5 border border-sky-100 overflow-hidden">
      <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-sky-50/30">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-sky-500 to-sky-600 text-white rounded-xl font-semibold text-sm shadow-lg shadow-sky-500/30 hover:shadow-xl hover:shadow-sky-500/40 hover:-translate-y-0.5 transition-all"
        >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold transition-all ${
                isActive
                  ...
                  : 'bg-white text-slate-600 border border-slate-200 hover:border-sky-300 hover:text-sky-600 hover:bg-sky-50'
              }`}
            >
              ...
              return (
                <React.Fragment key={category}>
                  ...
                  {group.rows.map((row, rowIdx) => (
                    <tr key={rowIdx} className={`${rowIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-sky-50/50 border-l-4 border-transparent hover:border-sky-400 transition-all`}>
                        ...
                        return (
                          <td key={cellIdx} className="px-5 py-4 text-sm border-r border-slate-100 last:border-r-0">
                            {cellIdx === 0 ? (
                              ...
                              cell && cell !== '—' ? (
                                <span className="inline-flex px-3 py-1 rounded-lg text-xs font-semibold bg-sky-50 text-sky-700 border border-sky-200">{cell}</span>
      ...
      <div className="px-5 py-3 bg-gradient-to-r from-sky-50/50 to-slate-50 border-t border-slate-100 flex justify-between items-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 mt-6 pt-6 border-t border-current/10">
          ...
          {healthData.added_count !== undefined && (
            <div className="text-center p-4 bg-sky-50 rounded-xl border border-sky-100">
              <div className="text-2xl font-bold text-sky-500">{healthData.added_count}</div>
