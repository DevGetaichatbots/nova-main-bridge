...
const MiniSpinner = () => (
  <svg className="w-4 h-4 animate-spin text-[#1eb5ee]" fill="none" viewBox="0 0 24 24">
...
const ScheduleAnalysisSidebar = ({
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'completed':
        return (
          <div className="w-2 h-2 rounded-full bg-[#1eb5ee] flex-shrink-0" />
        );
      case 'processing':
        return (
      ...
      <div className="p-4 border-b border-slate-200">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#1eb5ee] to-[#00B4B4] flex items-center justify-center">
        ...
        <button
          ...
          disabled={isCreating}
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#1eb5ee] to-[#00B4B4] text-white font-medium text-sm shadow-md hover:shadow-lg transition-all disabled:opacity-50"
        >
          {isCreating ? (
            <>
              <MiniSpinner />
            ...
            return (
              <div
                ...
                className={`group relative rounded-xl px-3 py-2.5 cursor-pointer transition-all duration-200 ${
                  ...
                  isActive
                    ? 'bg-[#1eb5ee]/10 border border-[#1eb5ee]/30 shadow-sm'
                    ...
                    {isRenaming ? (
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          autoFocus
                          className="flex-1 text-sm px-2 py-0.5 rounded border border-[#1eb5ee]/50 focus:outline-none focus:ring-1 focus:ring-[#1eb5ee] bg-white"
                          onClick={(e) => e.stopPropagation()}
                        />
                        <button
                          onClick={(e) => { e.stopPropagation(); handleSaveRename(analysis.analysis_id); }}
                          disabled={isSavingRename}
                          className="p-0.5 rounded hover:bg-[#1eb5ee]/15"
                        >
